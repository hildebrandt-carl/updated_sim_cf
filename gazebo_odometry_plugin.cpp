...
void GazeboOdometryPlugin::OnUpdate(const common::UpdateInfo& _info) {
  ...
  ignition::math::Pose3d W_pose_W_C = ignitionFromGazeboMath(link_->GetWorldCoGPose());
  ignition::math::Vector3d C_linear_velocity_W_C = ignitionFromGazeboMath(link_->GetRelativeAngularVel());
  ignition::math::Vector3d C_angular_velocity_W_C = ignitionFromGazeboMath(link_->GetRelativeAngularVel());
  ...
  if (parent_frame_id_ != kDefaultParentFrameId) {
    ignition::math::Pose3d W_pose_W_P = ignitionFromGazeboMath(parent_link_->GetWorldPose());
    ignition::math::Vector3d P_linear_velocity_W_P = ignitionFromGazeboMath(parent_link_->GetRelativeLinearVel());
    ignition::math::Vector3d P_angular_velocity_W_P = ignitionFromGazeboMath(parent_link_->GetRelativeAngularVel());
